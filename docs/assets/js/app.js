(() => {
  ...
  function forecastPayload(record) {
    return record?.payload && typeof record.payload === "object" ? record.payload : record || {};
  }

  function forecastDays(record) {
    const payload = forecastPayload(record);
  ...
  function forecastUpdatedAt(record) {
    const payload = forecastPayload(record);
    return payload.updated || payload.generated_at || record?.updated || record?.generated_at || record?.last_success_at;
  }

  function setForecast(record) {
    state.currentForecast = record;
    const days = forecastDays(record);
    const updatedAt = forecastUpdatedAt(record);
    ...
    elements.forecastUpdated.textContent = updatedAt ? `${record?.historical ? "Emitido" : "Actualizado"}: ${formatDateTime(updatedAt)}` : "Fecha de emisión no informada";
    elements.forecastNotice.innerHTML = "";
    if (record?.historical) {
      elements.forecastNotice.innerHTML = `<div class="notice"><strong>Último pronóstico oficial disponible.</strong> Fue emitido el ${escapeHtml(formatDateTime(updatedAt))} y puede no representar las condiciones actuales.</div>`;
